...
Batasan: $1 \le T \le 20; 1 \le K \le B \le 1.000; \mathbf{0 \le A \le 1}$
...
\begin{itemize}
	...
	\item Jika terdapat tepat 1 teman laki-laki ($A = 1$):
		\begin{itemize}
			...
			\item selain itu, cetak \texttt{P} sebanyak $\min(B, K)$, \texttt{L}, kemudian diikuti \texttt{P} sebanyak $B - \min(B, K)$.
		\end{itemize}
\end{itemize}
